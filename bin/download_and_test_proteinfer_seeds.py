...
def download_and_extract_model(id):
    ...
    if not os.path.exists(model_file):
        ...
        model_id_dir = (
            f"noxpd2_cnn_swissprot_go_random_swiss-cnn_for_swissprot_go_random-{id}"
        )
        ...
        run_command(
            f"conda run -n proteinfer python bin/export_proteinfer.py --model-path {model_dir / model_id_dir} --output-dir {model_dir} --model-name GO --add-model-id",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run protein inference on multiple models",
    )
    parser.add_argument(
        "--ids",
        nargs="+",
