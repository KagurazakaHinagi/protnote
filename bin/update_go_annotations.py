...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update old GO annotations file using a new one.",
    )

    # Add arguments for file paths
    parser.add_argument(
        "--old-annotations-file-path",
        required=True,
        help="Path to the old annotations file",
    )
    parser.add_argument(
        "--new-annotations-file-path",
        required=True,
        help="Path to the new annotations file",
    )
    parser.add_argument(
        "--output-file-path",
        required=True,
        help="Path to save the updated annotations file",
    )
    ...
    update_annotations(
        args.old_annotations_file_path,
        args.new_annotations_file_path,
