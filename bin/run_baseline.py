...
def main(
    ...
    proteinfer_predictions_path = (
        results_dir / f"test_logits_{annotation_type}_{test_name}_proteinfer.h5"
    )
    protnote_labels_path = results_dir / f"test_1_labels_{test_name}_{model_name}.h5"
    output_file = (
        results_dir
        / f"test_logits_{annotation_type}_{test_name}_{label_embedding_model}_baseline.h5"
    )
    ...
    base_embeddings_path = (
        project_root / "data" / "embeddings" / f"{label_embeddings}.pt"
    )
    base_embeddings_idx_path = (
        project_root / "data" / "embeddings" / f"{label_embeddings}_index.pt"
    )
    ...
    zero_shot_pinf_logits = pd.read_hdf(
        proteinfer_predictions_path, key="logits_df",
    ).astype("float32")
    ...
    if annotation_type == "EC":
        ...
        new_embeddings_path = (
            project_root / "data" / "embeddings" / f"{label_embeddings_new}.pt"
        )
        new_embeddings_idx_path = (
            project_root / "data" / "embeddings" / f"{label_embeddings_new}_index.pt"
        )
    ...
    zero_shot_label_mapping = {
        zero_shot_embeddings_idx["id"].iloc[zero_shot_label_idx]: train_embeddings_idx[
            "id"
