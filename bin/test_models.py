...
from protnote.utils.configs import construct_absolute_paths, get_project_root, load_config
...
def main():
    ...
    if args.test_type in ["all", "baseline"]:
        ...
        for label_encoder, test_set in list(product(["BioGPT", "E5", "Qwen3"], ["TEST_EC_DATA_PATH_ZERO_SHOT"])):
    ...
    for model_file in args.model_files:
        ...
        UPDATED_TEST_COMMANDS = {
            k: v.replace(MODEL_PATH_TOKEN, model_file).replace(MODEL_NAME_TOKEN, model_name)
